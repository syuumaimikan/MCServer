...
### 4. 🧩 Mod & プラグイン統合管理 ＆ 📦 友達へのMod一括配布 (Modpack Exporter)
...
- **📦 友達へのMod配布パック作成**: サーバー内のModをまとめたクライアント用ZIPやModrinthパックを1クリックで自動生成。ダウンロードURLやDiscord/LINE用案内文を即時発行。
...
### 6. 🌐 ネットワーク & ⚡ UPnP 全自動ルーターポート開放
- **⚡ UPnP 自動ポート開放**: ルーター設定画面を開くことなく、ボタン1つでご家庭のWi-Fiルーターのポート（TCP 25565）を自動開放・閉鎖。
- **マルチプレイ招待アシスタント**: ローカルIP / パブリックIP自動取得、LINE / Discord 用の参加案内テキストのワンクリックコピー。
- **トンネリング解説**: ルーター設定不要の無料ツール（Playit.gg）完全解説。

### 7. 💻 Apple Terminal風 リアルタイムコンソール
- ANSIカラー対応の美しいログビューア。
- 検索フィルター（INFO / WARN / ERROR / COMMAND）。
- コマンド履歴（上下キー対応）＆ クイックコマンドチップ。

...
## 5. サーバーのModを友達にワンクリック配布する (Modpack Exporter)
サーバーに入っているModを、友達に手動で1つずつ教える必要はありません。CraftOSがクライアント用ZIPパックを自動生成します。

### 手順：
1. 「Mod & プラグイン」または「ネットワーク」画面で **「📦 友達にModを配布」** ボタンをクリックします。
2. モーダルが開き、以下のいずれかの方法で配布できます：
   - **📥 ZIP を直接ダウンロード**: あなたがZIPを保存し、Discord等で友達に送信。
   - **🔗 ダウンロードURL & 案内文をコピー**: 友達に案内文を送ると、友達がブラウザで開くだけでModパックをダウンロード可能。
3. 友達はダウンロードしたZIPを解凍し、中にある `mods` フォルダの中身を自分の `.minecraft/mods` に入れるだけで、あなたと全く同じMod環境が完成します。

---

## 6. よくあるModトラブルと依存関係の解決
...
- **対処法**: CraftOSの **「🩺 クラッシュ診断 (Doctor)」** 画面でワンクリック修復を行うか、**「⏳ Time Machine」** から問題のMod導入前に巻き戻してください。
